...
- **`src/main.rs`** — the `ub-review` binary core, 27,633 lines. The cleanup train (one pure-code-motion extraction per PR, reached step 59) has extracted 49 top-level modules + the `proof/` (10-file) and `sensors/` (5-file) subtrees, so most logic now lives in dedicated modules. Modules are wired via `mod x; pub(crate) use x::*;` glob re-exports so the large inline `mod tests` keeps resolving through `use super::{...}` (note: this keeps modules physically split but logically still one flat namespace — a real-decoupling refactor would replace the globs with explicit imports). `main()` dispatches 13 subcommands — `init`, `doctor`, `cache`, `plan`, `run`, `summary`, `post`, `audit-ci`, `setup-ci`, `quality-backfill`, `quality-github-outcomes`, `quality-github-collect`, `gate-check` — to `cmd_*` functions (some now extracted: `cmd_init` in `init.rs`, `cmd_gate_check` in `gate.rs`, the `cmd_quality_github_*` pair in `quality_github.rs`). Remaining seams worth extracting when touched: the inline `mod tests` (~19.6k lines), the CI subsystem (`cmd_audit_ci`/`cmd_setup_ci` + ~25 `ci_*` helpers, ~2,300 lines). When touching `main.rs`, prefer extracting coherent seams over growing the monolith.
