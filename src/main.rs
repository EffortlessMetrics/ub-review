...
fn cmd_audit_ci(args: AuditCiArgs) -> Result<()> {
...
fn resolve_ci_audit_repo(args: &AuditCiArgs) -> Result<String> {
...
fn ci_repo_slug_from_remote_url(url: &str) -> Option<String> {
...
fn scan_local_workflows(root: &Path) -> Result<Vec<CiWorkflowScan>> {
...
fn ci_yaml_strip_comment(line: &str) -> &str {
...
fn ci_yaml_unquote(value: &str) -> String {
...
fn ci_yaml_inline_list(value: &str) -> Option<Vec<String>> {
    let trimmed = value.trim();
...
fn parse_ci_api_array<T: DeserializeOwned>(
...
fn ci_percentile(sorted: &[u64], pct: f64) -> Option<u64> {
    if sorted.is_empty() {
        return None;
    }
...
fn ci_round_rate(numerator: usize, denominator: usize) -> f64 {
    if denominator == 0 {
        return 0.0;
    }
...
fn ci_conclusion_blocks_independence(conclusion: Option<&str>) -> bool {
    !matches!(
        conclusion,
...
fn compute_ci_job_stats(workflows: &[CiApiWorkflow], runs: &[CiRunWithJobs]) -> Vec<CiJobStats> {
...
fn ci_security_pattern_match(candidates: &[&str]) -> Option<String> {
...
fn format_ci_duration(seconds: u64) -> String {
    if seconds >= 60 {
...
const CI_AUDIT_REPORT_TIER_SECTIONS: &[(&str, &str)] = &[
...
fn write_ci_audit_artifacts(dir: &Path, artifacts: &CiAuditArtifacts) -> Result<()> {
