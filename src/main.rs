...
";
        ...
        let tasks = super::focused_test_candidates_from_diff(&diff, &proof_requests, 3);
        assert_eq!(tasks.len(), 2);
...
"
        ...
        let proof_result = super::run_focused_red_green_proof_tasks_with_runner(
            ...
            tasks,
            |_root, _argv, _timeout, _stdout, _stderr| -> Result<CommandStatus> {
                runner_calls += 1;
                unreachable!("proof command should not run without a lease")
            },
            |_root, _out, _diff| -> Result<PathBuf> {
