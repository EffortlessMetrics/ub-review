...
const CI_AUDIT_SECURITY_PATTERNS: &[&str] = &[
    "codeql",
    "secret",
    "scan",
    "sign",
...
const CI_AUDIT_RUN_PAGE_CAP: usize = 10;
const CI_AUDIT_RUNS_PER_PAGE: usize = 100;
const CI_AUDIT_MIN_HISTORY_RUNS: usize = 20;
const CI_AUDIT_CHEAP_P50_SEC: u64 = 300;
const CI_AUDIT_HIGH_VOLUME_RUNS: usize = 100;
const CI_AUDIT_MEDIUM_VOLUME_RUNS: usize = 50;
...
struct CiCostsJob {
    ...
    matrix_expansion: usize,
}

#[derive(Debug, Serialize)]
struct CiCostsArtifact {
    schema: String,
    repo: String,
    window_days: u32,
    jobs: Vec<CiCostsJob>,
...
#[derive(Debug, Serialize)]
struct CiCorrelationArtifact {
    schema: String,
    repo: String,
    window_days: u32,
    independent_failure_rule: String,
    jobs: Vec<CiCorrelationJob>,
    evidence_gaps: Vec<String>,
}
...
fn render_setup_ci_branch_protection_doc(
    inventory: &CiInventoryArtifact,
    recommendations: &CiRecommendationsArtifact,
    required_check: &str,
) -> String {
...
fn render_setup_ci_gate_workflow(action_sha: &str, required_check: &str) -> String {
    format!(
...
struct SetupCiGeneratedFile {
    path: &'static str,
    content: String,
    message: &'static str,
}

fn setup_ci_generated_files(
    plan: &str,
...
fn valid_setup_ci_action_sha(args: &SetupCiArgs) -> Result<Option<&str>> {
...
fn write_setup_ci_preview_files(dir: &Path, files: &[SetupCiGeneratedFile]) -> Result<Vec<String>> {
...
struct SetupCiOpenContext<'a> {
    token: &'a str,
    out_dir: &'a Path,
}

fn setup_ci_api_post(
...
"#;
    ...
    fn ci_fixture_correlation_runs() -> Result<Vec<super::CiRunWithJobs>> {
        ci_fixture_runs(&[
            (
                101,
                vec![
    ...
    fn ci_workflow_scan_extracts_triggers_paths_timeouts_and_uses() -> Result<()> {
        let scan = super::scan_workflow_text(".github/workflows/ci.yml", CI_FIXTURE_WORKFLOW);
    ...
    fn ci_parse_api_array_counts_dropped_items() {
        ...
        let (parsed, dropped): (Vec<super::CiApiWorkflow>, usize) =
            super::parse_ci_api_array(&value, "workflows");
        assert_eq!(parsed.len(), 1);
        assert_eq!(dropped, 2);
        let (parsed, dropped): (Vec<super::CiApiWorkflow>, usize) =
            super::parse_ci_api_array(&serde_json::json!({}), "workflows");
    ...
    fn ci_percentile_uses_nearest_rank() {
        ...
        assert_eq!(super::ci_percentile(&values, 90.0), Some(90));
        assert_eq!(super::ci_percentile(&values, 99.0), Some(99));
        assert_eq!(super::ci_percentile(&[], 50.0), None);
    }

    #[test]
    fn ci_independent_failures_require_all_cheaper_jobs_passing() -> Result<()> {
        let stats =
            super::compute_ci_job_stats(&ci_fixture_workflows(), &ci_fixture_correlation_runs()?);
        let e2e = stats
            .iter()
    ...
    fn ci_security_jobs_always_flag_for_human() {
        let decision = super::classify_ci_job_tier(
        ...
        for job in [
            ...
            let decision =
                super::classify_ci_job_tier(&ci_tier_evidence(job, 500, 40, Some(60)), true);
    ...
    fn ci_expensive_quiet_job_right_sizes_to_adaptive_by_data_volume() {
        let decision =
            super::classify_ci_job_tier(&ci_tier_evidence("integration", 150, 0, Some(900)), true);
        assert_eq!(decision.tier, "adaptive");
        assert_eq!(decision.confidence, "medium");
        let decision =
            super::classify_ci_job_tier(&ci_tier_evidence("integration", 60, 0, Some(900)), true);
        assert_eq!(decision.tier, "adaptive");
        assert_eq!(decision.confidence, "low");
    }
    ...
    fn ci_survivorship_caps_confidence_without_proven_sibling_signal() {
        let decision =
            super::classify_ci_job_tier(&ci_tier_evidence("integration", 150, 0, Some(900)), false);
        assert_eq!(decision.tier, "adaptive");
        assert_eq!(decision.confidence, "low");
    ...
    fn ci_insufficient_history_is_never_adaptive() {
        let decision =
            super::classify_ci_job_tier(&ci_tier_evidence("integration", 5, 0, Some(900)), true);
        assert_eq!(decision.tier, "flag-for-human");
        assert_eq!(decision.confidence, "low");
    ...
    fn ci_audit_repo_empty_override_falls_back_to_git_origin() -> Result<()> {
        ...
        let args = |repo: Option<&str>| super::AuditCiArgs {
            root: repo_dir.clone(),
            out: temp.path().join("out"),
        ...
        assert_eq!(
            super::resolve_ci_audit_repo(&args(Some("")))?,
            "acme/widgets"
        );
        assert_eq!(
            super::resolve_ci_audit_repo(&args(Some("   ")))?,
        ...
        assert_eq!(super::resolve_ci_audit_repo(&args(None))?, "acme/widgets");
        assert_eq!(
            super::resolve_ci_audit_repo(&args(Some("acme/explicit")))?,
            "acme/explicit"
        );
        assert!(super::resolve_ci_audit_repo(&args(Some("not a slug"))).is_err());
        Ok(())
    }
