...
fn cmd_enable_with_resolver(args: EnableArgs, resolve: fn() -> ReleaseLookup) -> Result<()> {
    ...
    let config = if args.inspect {
        let inspection = inspect_init_guide_repo(root)?;
        render_enable_config_for_inspection(&inspection)
    } else {
...
fn require_inspection_root(root: &Path) -> Result<()> {
    if !root.is_dir() {
        bail!(
            "{} is not a directory; --inspect requires a repository root",
            root.display()
        );
    }
    Ok(())
...
/// Render the adoption config after the repository has been inspected. The
/// workflow remains the same, while the config records only sensors and lanes
/// that have a relevant local surface. This keeps first-run onboarding useful
/// for Rust, JavaScript/TypeScript, and mixed repositories without turning
/// heuristic inspection into required proof policy.
pub(crate) fn render_enable_config_for_inspection(inspection: &InitGuideInspection) -> String {
    let has_cargo = init_inspection_has_cargo(inspection);
    let has_javascript = inspection
        .build_systems
        .iter()
        .any(|system| system.starts_with("JavaScript/TypeScript"));
    let repo_kind = inspected_repo_kind(has_cargo, has_javascript);

    // `enable` writes the generated ub-review workflow immediately after this
    // config. Treat that post-write workflow as an inspected surface so the
    // first generated config does not disable its own gate-semantics sensors.
    let generated_workflow_surface = true;
    let mut config = format!(
        r#"# ub-review config generated by `ub-review enable --inspect`.
# The repository scan selects relevant sensors and review lanes only. It does
# not promote model suggestions or heuristics into required proof policy.
...
# Post actionable findings (severity medium+ or confidence medium-high+) to
# the PR; suppress pure lane-status boilerplate.
[review_body]
summary_only_body = "post_substantive"
...
[tools.cargo-allow]
enabled = {has_cargo_allow}

[tools.unsafe-review]
enabled = {has_unsafe}

[tools.actionlint]
enabled = {has_workflows}

[tools.zizmor]
enabled = {has_workflows}
"#,
        has_cargo_allow = inspection.cargo_allow_path.is_some(),
        has_unsafe = inspection.unsafe_native_found,
        has_workflows = generated_workflow_surface || !inspection.workflows.is_empty(),
    );

    if inspection.rust_source_count > 0 || has_javascript {
        append_enable_lane(
            &mut config,
            "source-route",
            "Changed source route review",
            ...
            &if has_cargo {
                vec!["tokmd", "ast-grep", "ripr"]
            } else {
                vec!["tokmd", "ast-grep"]
            },
            &["source-general", "source-ub", "tests-only"],
        );
    }
            ...
            .any(|script| script.name == "test")
    {
        append_enable_lane(
            ...
            "Check whether changed tests distinguish old from new behavior and request only focused proof that can change the review disposition.",
            &if has_cargo {
                vec!["tokmd", "ripr"]
            } else {
                vec!["tokmd"]
    ...
    if generated_workflow_surface || !inspection.workflows.is_empty() {
        append_enable_lane(
            &mut config,
            "gate-semantics",
            "Workflow and gate contract review",
            "Check workflow permissions, triggers, pinning, fork safety, and whether the generated gate remains advisory or required exactly as configured.",
    ...
    if inspection.docs_or_specs_count > 0 {
        append_enable_lane(
            ...
            "Keep onboarding and support claims tied to implemented behavior, receipts, and explicit operator decisions; do not invent proof or branch-protection changes.",
            &["tokmd"],
            &["docs-only", "source-general", "workflow/tooling"],
        );
    }
...
fn inspected_repo_kind(has_cargo: bool, has_javascript: bool) -> &'static str {
    match (has_cargo, has_javascript) {
        (true, true) => "mixed",
        (true, false) => "rust",
        (false, true) => "javascript-typescript",
        (false, false) => "generic",
    }
}

fn append_enable_lane(
    config: &mut String,
    id: &str,
    role: &str,
    focus: &str,
    ...
    config.push_str(&format!("focus = \"{focus}\"\n"));
...
mod tests {
    ...
    #[test]
    fn inspected_enable_config_selects_relevant_rust_surfaces() -> Result<()> {
        ...
        fs::write(root.join("docs/specs/contract.md"), "# Contract\n")?;
        fs::write(root.join("policy/cargo-allow.toml"), "# ledger\n")?;

        let inspection = inspect_init_guide_repo(root)?;
        let toml = render_enable_config_for_inspection(&inspection);
        let config = Config::from_toml_with_policy_receipts(&toml)?;

        assert_eq!(config.repo.kind, "rust");
        assert_eq!(config.providers.policy, "primary-with-fallback");
        assert!(config.tools["ripr"].enabled);
        ...
        assert_eq!(source_lane.id, "source-route");
        ...
        for lane in [
            ...
            assert!(
                config.lanes.iter().any(|candidate| candidate.id == lane),
    ...
    #[test]
    fn inspected_enable_config_disables_unavailable_sensors() -> Result<()> {
        let temp = tempfile::tempdir()?;
        let root = temp.path();
        fs::write(
        ...
        assert_eq!(config.repo.kind, "javascript-typescript");
        assert!(!config.tools["ripr"].enabled);
        assert!(!config.tools["cargo-allow"].enabled);
        assert!(!config.tools["unsafe-review"].enabled);
        assert!(config.tools["actionlint"].enabled);
        assert!(config.tools["zizmor"].enabled);
        assert!(config.lanes.iter().any(|lane| lane.id == "source-route"));
        assert!(config.lanes.iter().any(|lane| lane.id == "tests-red-green"));
        assert!(config.lanes.iter().any(|lane| lane.id == "gate-semantics"));
        Ok(())
    }

    #[test]
    fn enable_inspect_requires_a_repository_root() -> Result<()> {
        let temp = tempfile::tempdir()?;
        let args = EnableArgs {
        ...
        let error = cmd_enable_with_resolver(args, offline_resolve_source)
            .err()
            .ok_or_else(|| anyhow::anyhow!("--inspect should reject a missing root"))?;
        assert!(
            error
                .to_string()
        ...
        assert!(require_inspection_root(&temp.path().join("missing")).is_err());
        Ok(())
    }

    #[test]
    fn inspected_repo_kind_covers_all_repository_surfaces() {
        assert_eq!(inspected_repo_kind(true, true), "mixed");
        assert_eq!(inspected_repo_kind(true, false), "rust");
        assert_eq!(inspected_repo_kind(false, true), "javascript-typescript");
        assert_eq!(inspected_repo_kind(false, false), "generic");
    }
