...
fn handle_fake_ci_required_checks_request(mut stream: TcpStream) -> Result<String> {
    ...
    let response_body = if request_line == "GET /repos/acme/widgets HTTP/1.1" {
        ...
        .contains("/repos/acme/widgets/branches/release%2F2026/protection/required_status_checks")
    {
...
fn ci_fixture_run_with_jobs(id: u64, jobs: Vec<serde_json::Value>) -> Result<crate::CiRunWithJobs> {
...
fn ci_security_jobs_always_flag_for_human() {
    let decision =
        crate::classify_ci_job_tier(&ci_tier_evidence("CodeQL analyze", 500, 40, Some(60)), true);
    ...
    for job in [
        ...
        let decision = crate::classify_ci_job_tier(&ci_tier_evidence(job, 500, 40, Some(60)), true);
...
fn ci_cheap_job_with_independent_failures_keeps_required() {
    let decision = crate::classify_ci_job_tier(&ci_tier_evidence("fmt", 200, 4, Some(45)), false);
    assert_eq!(decision.tier, "keep-required");
...
fn setup_ci_print_pr_renders_sections_policy_and_round_trips() -> Result<()> {
    ...
    assert!(
        plan.contains("judgment: deterministic; receipts: ci-audit/correlation.json#integration")
    );
...
fn setup_ci_open_pr_creates_branch_files_and_pr_with_receipts() -> Result<()> {
    ...
    let mut preview_args = setup_ci_args(&out, vec!["integration=cargo test --locked".to_owned()]);
    ...
    let accepts = crate::parse_setup_ci_accepts(&["integration=cargo test --locked".to_owned()])?;
    let plan = fs::read_to_string(out.join("ci-audit/migration-plan.md"))?;
...
fn ci_audit_artifacts_carry_schema_fields_and_receipts() -> Result<()> {
    ...
    for name in [
        ...
        let value: serde_json::Value = serde_json::from_slice(&fs::read(temp.path().join(name))?)?;
...
fn ci_audit_tokenless_degrades_to_inventory_only() -> Result<()> {
    ...
    let artifacts =
        crate::build_ci_audit_artifacts("acme/widgets", 90, &scans, None, None, crate::Utc::now());
