...
pub(crate) fn write_plan_artifacts(
        ...
        && let Some(base_tree) = run_args.review.trusted_base_tree.as_deref()
    {
        let receipt = serde_json::json!({
                ...
                "changed_files_object_sha256": run_args.review.trusted_changed_files.as_deref().map(fs::read).transpose()?.map(|bytes| sha256_hex(&bytes)),
                "patch_object_sha256": run_args.review.trusted_patch.as_deref().map(fs::read).transpose()?.map(|bytes| sha256_hex(&bytes)),
