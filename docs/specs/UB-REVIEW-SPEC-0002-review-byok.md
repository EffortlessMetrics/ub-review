...
- Runtime fallback retry (landed in PR #315): a lane that fails mid-run with
  ...
  fallback where one is configured. Any future decision to wire provider
  model/env/role or prompt-cache config remains under spec 0006; provider
  policy and per-provider `max_concurrency` are already executed.
...
1. Provider config remainder — any future provider model/env/role or
   prompt-cache config (lands under spec 0006).
