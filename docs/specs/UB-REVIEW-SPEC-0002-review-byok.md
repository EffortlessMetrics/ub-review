...
- Posting on quiet passes fails closed into silence: a pass whose event is
  ...
  `skipped_pass_policy` instead of posting. `[gate].synchronize_mode` is
  removed (#306); setting it yields a deprecation `PolicyError` receipt.
...
2. #311 — include proof receipt content in follow-up prompts (landed in
   PR #322).
3. #306 — DONE: the field is removed with a dedicated deprecation receipt;
   `post_review_on` is the only posting policy, now structurally.
